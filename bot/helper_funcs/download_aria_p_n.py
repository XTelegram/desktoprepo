...
async def aria_start():
    ...
    aria2_daemon_start_cmd = ["chrome", "--conf-path=/app/bot/aria2/aria2.conf", f"--rpc-listen-port={ARIA_TWO_STARTED_PORT}", f"--bt-stop-timeout={MAX_TIME_TO_WAIT_FOR_TORRENTS_TO_START}", f"--bt-tracker=[{TRACKERS}]"]
    ...
    return ariaAPI(
        ariClient(
            host="http://localhost", port=5200, secret=""
        )
    )
